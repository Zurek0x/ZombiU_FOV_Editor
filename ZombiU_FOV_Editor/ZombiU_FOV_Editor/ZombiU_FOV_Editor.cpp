...
using namespace std;
DWORD fov_addr = 0x00DA4CB4;
DWORD fov_lock = 0x0064A215;
int fov_data;
...
// in game FOV value is [ 10 for 1065749138 ] in 4 Bytes

// To disable FOV lock set [ 0064A215 ] Value to [ 1575324419 with 4 Byte Value]
// To restore FOV lock set [ 0064A215 ] Value to [ 1575324416 with 4 Byte Value ]

int disable_fov_lock = 1575324419;
int restore_fov_lock = 1575324416;
...
int main() {
	...
	// Controls //
	cout << "[ Controls ]\nRight Arrow = HIGHER FOV\nLeft Arrow = LOWER FOV" << endl;
	// Disable FOV Lock //
	WriteProcessMemory(pHandle, (LPVOID)fov_lock, &disable_fov_lock, (DWORD)sizeof(disable_fov_lock), 0);
	while (true) {
		// Sleep(scan_delay);
		ReadProcessMemory(pHandle, (void*)fov_addr, &fov_data, sizeof(fov_data), 0);
		if (GetAsyncKeyState(VK_RIGHT)) {
			fov = fov_data + increse_val;
			WriteProcessMemory(pHandle, (LPVOID)fov_addr, &fov, (DWORD)sizeof(fov), 0);
		}
		if (GetAsyncKeyState(VK_LEFT)) {
			fov = fov_data + decrese_val;
			WriteProcessMemory(pHandle, (LPVOID)fov_addr, &fov, (DWORD)sizeof(fov), 0);
		}
	}
}
